...
from backend.backend import backend  # 引入后端路由
from public.public import public_app
...
app.include_router(public_app, prefix="/public", tags=["公共请求"])  # 管理端默认根目录
...
@app.middleware("http")
async def verify_user_token(request: Request, call_next):
